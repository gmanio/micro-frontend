...
- [자산](/docs/asset/overview) — 사이트·호실·유닛·인벤토리
...
| **자산** | 사이트·호실·유닛 — [자산 도메인](/docs/asset/overview) |
...
계약은 자산 스냅샷(`site` · `dong` · `room` · `unit`)을 고정합니다.

| 경로 | 자산 | 인벤토리 |
|------|------|----------|
| Passport | **유닛** 선택 → 가용 **호실** 자동 배정 | 가계약금(`BOOKED`) 시 점유 |
| Admin 생성 | 사이트→유닛→동→호실 | `ADMINSTART` 즉시 점유 |
| Admin 호실 확정 | `BOOKED`에서 호실 확정 | inventory swap · `READIED` 경로 |

상세: [자산 overview](/docs/asset/overview) · [인벤토리](/docs/asset/inventory).
